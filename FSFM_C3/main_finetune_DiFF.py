...
def get_args_parser():
    ...
    parser.add_argument('--from_scratch', default=False,
                        help='train from scratch')
    parser.add_argument('--from_imagenet', default=False,
                        help='train from imagenet-pretrained')
...
def get_shared_folder() -> Path:
    ...
    if Path("checkpoint/").is_dir():
        p = Path(f"{cur_file_path}/{user}/experiments_finetune/DiFF")
        if args.eval:
            p = Path(f"{cur_file_path}/{user}/experiments_finetune/DiFF/test/")
...
def main(args):
    ...
    if args.from_scratch and not args.eval:
        args.finetune = False

    if args.from_imagenet and not args.eval:
    ...
    max_accuracy = {'T2I': 0.0, 'I2I': 0.0, 'FS': 0.0, 'FE': 0.0}
    max_auc = {'T2I': 0.0, 'I2I': 0.0, 'FS': 0.0, 'FE': 0.0}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for val_ds in ['T2I', 'I2I', 'FS', 'FE']:
            ...
            max_accuracy[val_ds] = max(max_accuracy[val_ds], val_stats["acc"])
            max_auc[val_ds] = max(max_auc[val_ds], val_stats["auc"])
            print(max_accuracy, max_auc)
