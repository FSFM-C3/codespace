...
def main(args):
    ...
    if args.normalize_from_IMN:
        ...
        # # to use FF++_youtube(faces from all c23 train and val frames):
    ...
    # define the target model/branch
    model_target_encoder = models_fsfm.vit_target_encoder(args.model)
    # init model_target_encoder
    interpolate_pos_embed_ema(model_target_encoder, model)
    msg = model_target_encoder.load_state_dict(model.state_dict(), strict=False)
    print(msg)
    # there is no backpropagation through the teacher, so no need for gradients
    for p in model_target_encoder.parameters():
        p.requires_grad = False

    model.to(device)
    model_target_encoder.to(device)
    ...
    model_target_encoder_without_ddp = model_target_encoder
    print("Model_target_encoder = %s" % str(model_target_encoder_without_ddp))
    ...
    misc.load_model_target_encoder(args=args, model_target_encoder_without_ddp=model_target_encoder_without_ddp,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model,
                                      momentum_schedule, model_target_encoder, model_target_encoder_without_ddp,
        ...
        if args.output_dir and (epoch % 50 == 0 or epoch + 1 == args.epochs):
            ...
            misc.save_model_target_encoder(
                args=args, model=model_target_encoder,
                model_target_encoder_without_ddp=model_target_encoder_without_ddp,
