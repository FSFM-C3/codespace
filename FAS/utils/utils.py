...
def sample_frames(flag, num_frames, dataset_name, args):
    """
    ...
    if dataset_name in ['casia', 'replay', 'oulu', 'msu', 'celeb']:
        root = '/FAS/data/MCIO/frame/'
        dataroot = '/FAS/data/MCIO/txt/'
    else:
        root = '/FAS/data/WCS/frame/'
        dataroot = '/FAS/data/WCS/txt/'
...
def sample_frames_v1(flag, num_frames, dataset_name):
    """
        from every video (frames) to sample num_frames to test
        return: the choosen frames' path and label
    """
    ...
    if dataset_name in ['casia', 'replay', 'oulu', 'msu', 'celeb']:
        root = 'data_dlib/MCIO/frame/'
        dataroot = 'data_dlib/MCIO/txt/'
    else:
        root = 'data_dlib/WCS/frame/'
        dataroot = 'data_dlib/WCS/txt/'

    if (flag == 0):  # select the fake images
        data = [[
            root + i.strip()
            for i in open(dataroot + dataset_name + '_fake_train.txt').readlines() if os.path.isfile(root + i.strip())
        ], []]
        print('train fake:', dataset_name, len(data[0]), len(data[1]))
        ...
        data = [[],
                [
                    root + i.strip() for i in open(dataroot + dataset_name +
                                                   '_real_train.txt').readlines() if os.path.isfile(root + i.strip())
                ]]
        print('train real:', dataset_name, len(data[0]), len(data[1]))
        ...
        data = [[
                    root + i.strip()
                    for i in open(dataroot + dataset_name + '_fake_shot.txt').readlines() if os.path.isfile(root + i.strip())
                ][:5], []]
        data = [data[0] + data[0] + data[0] + data[0], []]
        print('train fake:', dataset_name, len(data[0]), len(data[1]))
        ...
        data = [[],
                [
                    root + i.strip() for i in open(dataroot + dataset_name +
                                                   '_real_shot.txt').readlines() if os.path.isfile(root + i.strip())
                ][:5]]
        data = [[], data[1] + data[1] + data[1] + data[1]]
        print('train real:', dataset_name, len(data[0]), len(data[1]))
        ...
        data = [[
                    root + i.strip()
                    for i in open(dataroot + dataset_name + '_fake_test.txt').readlines() if os.path.isfile(root + i.strip())
                ] + [
                    root + i.strip().replace('frame0', 'frame1')
                    for i in open(dataroot + dataset_name + '_fake_test.txt').readlines() if os.path.isfile(root + i.strip())
                ],
                [
                    root + i.strip() for i in open(dataroot + dataset_name +
                                                   '_real_test.txt').readlines()
                    if os.path.isfile(root + i.strip().replace('frame0', 'frame1'))
                ] + [
                    root + i.strip().replace('frame0', 'frame1')
                    for i in open(dataroot + dataset_name +
                                  '_real_test.txt').readlines()
